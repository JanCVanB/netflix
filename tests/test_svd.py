...
def test_svd_update_feature_in_c_modifies_users_and_movies_as_expected():
    ...
    for feature in range(c_model.num_features):
        ...
        np.testing.assert_array_almost_equal(c_model.users, py_model.users, decimal=4)
        np.testing.assert_array_almost_equal(c_model.movies, py_model.movies, decimal=4)
